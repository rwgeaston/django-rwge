...
settings = {
    'mother_likelihood': settings.DEFAULT_MOTHER_FREQUENCY,
}


def get_response(text, author, mentioned):
    ...
    if 'how are you' in text:
        return random.choice(['that went badly', 'rubbish'])
...
def question_rules(random_value, text):
    ...
    if '?' not in text:
        ...
        response = "ask %s" % random.choice(settings.PEOPLE_TO_ASK)
...
def default_reply(text):
    ...
    if text[-1] == '?':
        response = random.choice([
            ...
            'seems reasonable',
            f'ask {random.choice(settings.PEOPLE_TO_ASK)}',
        ])
        return response
